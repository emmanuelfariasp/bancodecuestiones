...
function updateQuizSubtitle(){
  if(!$('quizSub') || !currentSection) return;
  const base=currentSection.subtitle || '';
  if(currentMode==='exam'){
    $('quizSub').textContent=`${base} · Modo simulado: pregunta ${idx+1} de ${questions.length}. No muestra aciertos ni corrección hasta finalizar.`;
  } else {
    $('quizSub').textContent=`${base} · Recibirás explicación inmediata.`;
  }
}
function renderQuestion(){
  ...
  $('nextBtn').textContent=(idx===questions.length-1?(currentMode==='exam'?'Finalizar y ver resultado':'Ver resultado'):'Siguiente');
  ...
  q.options.forEach((op,i)=>{
    ...
    b.className='option';
    b.innerHTML=`<b>${letters[i]})</b>&nbsp;${escHtml(op)}`;
    b.onclick=()=>selectOption(i);
    opts.appendChild(b);
  });
  restoreCurrentAnswerView();
...
function nextQuestion(){
  ...
  if(idx>=questions.length){ showResult(); return; }
  if(currentMode==='exam'){
    const existing=answerForCurrent();
    if(selectedChoice===null && !existing) return;
    if(selectedChoice!==null && (!existing || existing.choice!==selectedChoice)){
      storeAnswer(selectedChoice,{updateWrongBank:false});
    }
  }
  saveProgress();
  ...
  saveProgress();
  renderQuestion();
  if(currentMode!=='exam' && !$('questionList').classList.contains('hidden')) renderQuestionList();
}
